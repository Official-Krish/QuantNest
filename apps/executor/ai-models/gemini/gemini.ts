...
import {
  buildFallbackDailyPerformanceAnalysis,
  buildFallbackInsight,
} from "../fallbacks";
import {
  normalizeDailyPerformanceAnalysis,
  extractJsonBlock,
  normalizeInsight,
} from "../normalization";
import {
  buildDailyPerformancePrompt,
...
export class GeminiReasoningProvider implements AiReasoningProvider {
  ...
  async generateTradeReasoning(
    eventType: EventType,
    details: NotificationDetails,
  ): Promise<NotificationAiInsight> {
    if (!this.ai) return buildFallbackInsight(eventType, details);

    try {
      const defaultReferences =
        marketContextBuilder.buildDefaultReferences(details);
      const indicatorSnapshot =
        await marketContextBuilder.buildIndicatorSnapshot(
          details,
          defaultReferences,
        );

      const marketType = details.aiContext?.marketType || "Indian";
      const primarySymbols = [details.symbol, details.aiContext?.symbol].filter(
      ...
      const marketDataArray = await Promise.all(
        uniqueSymbols.map((symbol) =>
          marketContextBuilder.build(symbol, marketType).catch((error) => {
  ...
  async generateDailyPerformanceAnalysis(
