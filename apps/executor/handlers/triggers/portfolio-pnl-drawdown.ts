...
import { getBrokerAccountMetrics, type BrokerAccountMetrics } from "../../services/accountMetrics";
...
function normalizeRiskMode(value: unknown): RiskMode {
  const normalized = String(value || "daily-loss-cap").trim().toLowerCase();
...
function normalizeThresholdUnit(value: unknown): ThresholdUnit {
  return String(value || "absolute").trim().toLowerCase() === "percent" ? "percent" : "absolute";
}

function getPreviousRuntime(workflow: WorkflowType): PortfolioRiskRuntime {
  const runtime = (workflow.triggerConfig?.runtime || {}) as PortfolioRiskRuntime;
  return {
    baselineAccountValue: typeof runtime.baselineAccountValue === "number" ? runtime.baselineAccountValue : undefined,
    peakAccountValue: typeof runtime.peakAccountValue === "number" ? runtime.peakAccountValue : undefined,
    lastMeasuredAt: typeof runtime.lastMeasuredAt === "string" ? runtime.lastMeasuredAt : undefined,
    lastAccountValue: typeof runtime.lastAccountValue === "number" ? runtime.lastAccountValue : undefined,
    lastTotalPnl: typeof runtime.lastTotalPnl === "number" ? runtime.lastTotalPnl : undefined,
    lastDrawdown: typeof runtime.lastDrawdown === "number" ? runtime.lastDrawdown : undefined,
    lastDrawdownPct: typeof runtime.lastDrawdownPct === "number" ? runtime.lastDrawdownPct : undefined,
  };
}
...
export async function handlePortfolioPnlDrawdownTrigger(
  ...
  const broker = String(metadata.broker || "").trim().toLowerCase() as "zerodha" | "groww" | "lighter";
  if (!["zerodha", "groww", "lighter"].includes(broker)) {
    throw new Error("Portfolio PnL / Drawdown trigger requires broker to be zerodha, groww, or lighter.");
  }
  ...
  if (!Number.isFinite(thresholdValue) || thresholdValue <= 0) {
    throw new Error("Portfolio PnL / Drawdown thresholdValue must be greater than 0.");
  }
  ...
  const accountValue = typeof metrics.accountValue === "number" && Number.isFinite(metrics.accountValue)
    ? metrics.accountValue
    : undefined;
  const baselineAccountValue = previousRuntime.baselineAccountValue ?? accountValue;
  const peakAccountValue = accountValue === undefined
    ? previousRuntime.peakAccountValue
    : Math.max(previousRuntime.peakAccountValue ?? accountValue, accountValue);
  const drawdown = accountValue !== undefined && peakAccountValue !== undefined
    ? Math.max(0, peakAccountValue - accountValue)
    : undefined;
  const drawdownPct = drawdown !== undefined && peakAccountValue && peakAccountValue > 0
  ...
  if (mode === "daily-loss-cap") {
    if (thresholdUnit === "percent") {
      ...
      if (!base || base <= 0) {
        throw new Error("Daily loss percent threshold requires account value from broker metrics.");
      }
  ...
  if (mode === "profit-target") {
    if (thresholdUnit === "percent") {
      const base = accountValue ?? baselineAccountValue;
      if (!base || base <= 0) {
        throw new Error("Profit target percent threshold requires account value from broker metrics.");
      }
      measuredValue = Math.max(0, (metrics.totalPnl / base) * 100);
      shouldExecute = measuredValue >= thresholdValue;
    } else {
  ...
  if (mode === "drawdown-limit") {
    if (drawdown === undefined || drawdownPct === undefined) {
      throw new Error("Drawdown threshold requires account value from broker metrics.");
    }
