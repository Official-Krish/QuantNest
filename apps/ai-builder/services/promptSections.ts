import type { AiStrategyBuilderRequest } from "@quantnest-trading/types/ai";

type PlannerPromptSectionsParams = {
  input: AiStrategyBuilderRequest;
  allowedNodeTypes: string;
  preferredActions: string;
  constraints: string;
  actionMetadataGuide: string;
  triggerMetadataGuide: string;
};

export function buildPlannerPromptSections({
  input,
  allowedNodeTypes,
  preferredActions,
  constraints,
  ...
  return [
    ...
    "- BUPS (Break Up Pattern Structure / Confirmed Breakout): price breaks above level, retests, then confirms higher. User phrases: 'BUPS', 'confirmed breakout', 'breakout with retest', 'break up structure'. Use breakout-retest-trigger. Do NOT use plain price trigger for BUPS.",
    "- Daily Loss Cap: monitor account PnL daily. User phrases: 'daily loss cap', 'stop loss at X', 'day loss limit'. Use portfolio-pnl-drawdown-trigger with mode='daily-loss-cap'.",
    "- Profit Target: monitor for daily profit goal. User phrases: 'profit target', 'target profit at X', 'hit profit goal'. Use portfolio-pnl-drawdown-trigger with mode='profit-target'.",
    "- Drawdown Protection: monitor account drawdown %. User phrases: 'drawdown limit', 'drawdown protection', 'account drawdown'. Use portfolio-pnl-drawdown-trigger with mode='drawdown-limit'.",
    "",
    "BRANCHING LOGIC:",
