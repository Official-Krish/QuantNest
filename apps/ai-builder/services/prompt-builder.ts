...
export function buildStrategyPlannerPrompt(input: AiStrategyBuilderRequest): string {
  ...
  return [
    ...
    "- For market-session timing requests (e.g., 'Only trade between 9:20 and 2:30', 'Close workflow at 3:20'), use market-session trigger nodes with event='at-time' and triggerTime in HH:MM format.",
    "- market-session trigger metadata must include: marketType ('indian' or 'web3'), event ('market-open', 'market-close', 'at-time', or 'pause-at-time'), and triggerTime (HH:MM format when event is time-based).",
    ...
    "- Indian preset intent mapping: after first 5 minutes -> at-time 09:20; after first 15 minutes/opening range -> at-time 09:30; avoid first 10 minutes -> at-time 09:25; avoid 12:00-13:30 -> at-time 13:30; stop taking new trades after 15:00 -> pause-at-time 15:00; close all positions by 15:20 -> at-time 15:20 plus appropriate action nodes.",
    "- Crypto preset intent mapping: trade only during US session -> market-session at-time around 19:00 IST, then use downstream filter/if logic for strategy conditions.",
