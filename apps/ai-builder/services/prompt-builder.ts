...
export function buildStrategyPlannerPrompt(input: AiStrategyBuilderRequest): string {
  ...
  return [
    ...
    "- market-session trigger metadata must include: marketType ('indian' or 'web3'), event ('market-open', 'market-close', or 'at-time'), and triggerTime (HH:MM format when event='at-time').",
    "- For time-based workflow execution like 'trade between X and Y', use multiple market-session triggers (one to start at X time, one to stop at Y time).",
