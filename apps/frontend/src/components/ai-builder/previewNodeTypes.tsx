  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        <span
          className={`rounded-full px-2 py-0.5 text-[8px] font-semibold uppercase tracking-[0.18em] ${
            tone === "trigger"
              ? "bg-[#f17463]/16 text-[#ffbaa8]"
              : "bg-[#f17463]/20 text-[#ffd1c8]"
          }`}
        >
          {kindBadge}
        </span>
      ...
      <div className="flex items-center justify-between gap-2">
        <span
          className="inline-flex items-center gap-1.5 text-[8px] font-semibold uppercase tracking-[0.18em]"
          style={{ color: accent }}
        >
      ...
      <div
        className={
          "mt-1.5 text-[12px] font-semibold leading-4.5 text-neutral-100 " +
          (titleClassName || "")
        }
      >
        {title}
      ...
      {subtitle ? (
        <div className="mt-1 text-[10px] leading-4 text-neutral-300/80">
          {subtitle}
        </div>
      ) : null}
      {showTarget ? (
...
export function PreviewPriceTrigger({ data }: any) {
  ...
  const subtitle =
    mode === "change" ? `Trigger on ${asset} momentum` : `Trigger on ${asset}`;
  return (
    <PreviewShell
      accent="#f17463"
      tone="trigger"
...
export function PreviewConditional({ data }: any) {
  ...
  const firstGroup =
    expression?.conditions?.find((entry: any) => entry?.type === "group") ||
  ...
  return (
    <div className="min-w-34 rounded-2xl border border-[#f17463]/35 bg-[#f17463]/7 px-3 py-2.5 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="text-[8px] font-semibold uppercase tracking-[0.18em] text-[#ffbaa8]">
          Condition
        </span>
        <span className="rounded-full bg-black/50 px-1.5 py-0.5 text-[8px] font-mono text-neutral-200">
          {groups}G
      ...
      <div className="mt-1.5 text-[12px] font-semibold leading-4.5 text-neutral-100">
        Branch workflow
      </div>
      <div className="mt-1 text-[10px] leading-4 text-neutral-300/80">
        {subtitle}
      ...
      <Handle
        ...
        id="true"
        position={Position.Right}
        className="h-2! w-2! border! border-neutral-900! bg-emerald-400!"
        style={{ top: "40%" }}
      />
      <Handle
        type="source"
        id="false"
        position={Position.Right}
        className="h-2! w-2! border! border-neutral-900! bg-rose-400!"
        style={{ top: "68%" }}
      />
    </div>
  );
...
export function PreviewMarketSession({ data }: any) {
  ...
  const marketLabel =
    String(marketType).toLowerCase() === "web3" ? "Crypto" : "Indian";
  const title =
    event === "market-close"
      ? "Market close"
...
export function PreviewPortfolioPnlDrawdownTrigger({ data }: any) {
  ...
  const brokerLabel =
    String(broker).charAt(0).toUpperCase() + String(broker).slice(1);
...
export function PreviewIf({ data }: any) {
  ...
  return (
    <div className="min-w-34 rounded-2xl border border-[#f17463]/45 bg-[#f17463]/13 px-3 py-2.5 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="text-[8px] font-semibold uppercase tracking-[0.18em] text-[#f59a8d]">
          If
        </span>
        <span className="rounded-full bg-black/50 px-1.5 py-0.5 text-[8px] font-mono text-neutral-200">
          {groups}G
...
export function PreviewRecheck({ data }: any) {
  const seconds = Number(data.metadata?.durationSeconds || 0);
  const mode =
    String(data.metadata?.recheckMode || "trigger").toLowerCase() === "custom"
      ? "Custom"
      : "Trigger";
  return (
    <div className="min-w-34 rounded-2xl border border-[#f17463]/45 bg-[#f17463]/13 px-3 py-2.5 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
...
export function PreviewZerodha({ data }: any) {
  ...
  return (
    <PreviewShell
      ...
      subtitle={
        hasSecret
          ? "Broker order · credentials from stored secret"
          : "Broker order"
      }
      iconService="zerodha"
    />
...
export function PreviewGmail({ data }: any) {
  const { recipientName = "User", recipientEmail = "No email" } =
    data.metadata || {};
  return (
    <PreviewShell
      accent="#ea4335"
...
export function PreviewNotion({ data }: any) {
  ...
  return (
    <PreviewShell
      ...
      title="Daily report"
      subtitle={subtitle}
      iconService="notion-daily-report"
    />
  );
}
...
export function PreviewGoogleDrive({ data }: any) {
  ...
  const hasSecret = Boolean(String(secretId || "").trim());
  const subtitle = hasSecret
    ? "Reporting · credentials from stored secret"
    : "Reporting";
  return (
    <PreviewShell
      accent="#8ab4f8"
      ...
      subtitle={subtitle}
      iconService="google-drive-daily-csv"
    />
  );
}
...
export function PreviewGoogleSheets({ data }: any) {
  ...
  return (
    <PreviewShell
      ...
      subtitle={subtitle}
      iconService="google-sheets-report"
    />
  );
}
...
export function PreviewPostgres({ data }: any) {
  ...
  const host =
    String(connectionString || "")
      ...
      .split("/")[0]
      .split("@")[1] || "No host";
  const subtitle = tableName ? `Table: ${tableName}` : "Missing table";
  return (
    <PreviewShell
      accent="#34d399"
      tone="action"
