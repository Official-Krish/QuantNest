...
export const filterAction = ({
  ...
  return (
    <div className="min-w-[230px] rounded-2xl border border-neutral-700/80 border-l-[5px] border-l-[#14b8a6] bg-neutral-950/90 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_0_0_1px_rgba(255,255,255,0.04)]">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#5eead4]">
          Filter
        </span>
        ...
        <div className="mt-2 space-y-2">
          ...
          {expression.conditions.map((groupOrClause: any, index: number) => {
            const group = groupOrClause?.type === "group"
              ? groupOrClause
              : { type: "group", operator: "AND", conditions: [groupOrClause] };
            return (
              <div key={`group-${index}`} className="space-y-1 rounded-lg border border-neutral-800 bg-neutral-900/40 p-2">
                {index > 0 && (
                  <div className="flex justify-center">
                    <span className="rounded-full border border-neutral-700 bg-neutral-900 px-2 py-0.5 text-[9px] font-semibold text-[#5eead4]">
                      {expression.operator}
                    </span>
                  </div>
                )}
        ...
        <div className="mt-2 rounded-lg border border-neutral-800 bg-black/40 px-2.5 py-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-[9px] font-semibold uppercase tracking-[0.14em] ${preview?.conditionMet ? "text-emerald-300" : "text-neutral-400"}`}>
              {loading ? "Fetching" : preview?.conditionMet ? "Passing" : "Blocked"}
            </span>
          </div>
      ...
      <div className="mt-1 text-[11px] text-neutral-400">
        Continues only when the condition passes.
      </div>
      <Handle
        type="target"
      ...
      <Handle
        ...
        className="h-2! w-2! bg-[#14b8a6]! border border-neutral-900"
      />
    </div>
  );
};
