...
workFlowRouter.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Workflow created", workflowId: workflow._id });
  } catch (error) {
    ...
    if (
      await handleWorkflowBrokerVerificationFailure({
        userId,
        workflowName: data.workflowName,
        stage: "create",
        error,
...
workFlowRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      const result = await getExecutionTrace(executionId, userId);
      if (!result) {
