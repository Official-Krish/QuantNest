...
function getReusableSecretKey() {
  const raw =
    process.env.REUSABLE_SECRET_ENCRYPTION_KEY || process.env.JWT_SECRET || "";
  if (!raw) {
...
export function encryptReusableSecretPayload(
...
export function decryptReusableSecretPayload(
...
AiStrategyDraftVersionSchema.index(
  { userId: 1, draftId: 1, versionId: 1 },
  { unique: true },
);
AiStrategyDraftVersionSchema.index({ draftId: 1, createdAt: 1 });
WorkflowSchema.index({ status: 1, triggerType: 1, nextRunAt: 1 });
UserReusableSecretSchema.index(
  { userId: 1, service: 1, name: 1 },
  { unique: true },
);
...
export const WorkflowExampleModel = mongoose.model(
  "WorkflowExamples",
  WorkflowExampleSchema,
);
export const AiStrategySessionModel = mongoose.model(
  "AiStrategySessions",
  AiStrategySessionSchema,
);
export const AiStrategyDraftVersionModel = mongoose.model(
  "AiStrategyDraftVersions",
  AiStrategyDraftVersionSchema,
);
